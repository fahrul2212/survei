...
export default function App() {
    ...
    return (
      <main className="grid min-h-[100dvh] place-items-center bg-slate-50 p-5">
        <section className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 md:p-8">
          <Logo />
          <p className="mt-8 text-[11px] font-bold uppercase tracking-wider text-[#d91f17]">Company invitation</p>
          <h1 className="mt-1 text-3xl font-bold tracking-tight text-slate-900">Confirm your invitation</h1>
          <p className="mt-3 text-sm leading-6 text-slate-600">Continue only if you expected an invitation to the STICA reporting portal. Your one-time credential is not used until you confirm.</p>
          {inviteError && <p className="mt-5 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm font-medium text-red-700">{inviteError}</p>}
          <div className="mt-7 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            <Button variant="secondary" onClick={() => { window.history.replaceState({}, "", "/"); window.location.reload(); }}>Cancel</Button>
            <Button variant="primary" disabled={inviteBusy} onClick={async () => {
              setInviteBusy(true);
              setInviteError("");
              try {
                const verified = await client.auth.verifyOtp({ token_hash: inviteToken, type: "invite" });
                if (verified.error || !verified.data.session) throw verified.error ?? new Error("Invitation could not be verified");
                window.history.replaceState({}, "", "/");
                setSession(verified.data.session);
              } catch (error) {
                setInviteError(error instanceof Error ? error.message : "This invitation is invalid or has expired.");
              } finally { setInviteBusy(false); }
            }}>{inviteBusy ? "Confirming…" : "Accept invitation"}</Button>
          </div>
        </section>
      </main>
    );
  ...
  if (invitation) {
    return <InvitationSetup invitation={invitation} onComplete={async () => {
        ...
        <section className="flex w-full max-w-md flex-col gap-6 rounded-2xl border border-slate-200 bg-white p-8">
          ...
          <form
            ...
            onSubmit={async (e) => {
              ...
              if (p.length < 12 || !/[A-Za-z]/.test(p) || !/\d/.test(p)) return setRecoveryError("Use at least 12 characters with letters and a number.");
          ...
          >
            ...
            {recoveryError && <p className="rounded-lg border border-red-200 bg-red-50 px-3.5 py-2.5 text-sm font-medium text-red-700">{recoveryError}</p>}
